...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "UniverseCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    ...
    {"xurious.com", "dnsseed.UNIC.xurious.com"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. UniverseCoin is probably already running."), addrBind.ToString().c_str());
        else
