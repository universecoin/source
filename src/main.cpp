...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UniverseCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UniverseCoin Signed Message:\n";
...
{
    // UniverseCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2100000); // UniverseCoin: 2100000 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 1 * 60 * 60; // UniverseCoin: 1 hour retarget
static const int64 nTargetSpacing = 1 * 60; // UniverseCoin: 1 minute block
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // UniverseCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // UniverseCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1388534401, nBits=1e0ffff0, nNonce=2084524493, vtx=1)
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // UniverseCoin: increase each by adding 2 to bitcoin's value.
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    RenameThread("UniverseCoin-miner");
